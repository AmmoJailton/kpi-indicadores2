...
class DailyReportEndpoint(IEndpoint):
    @property
    def endpoints(self) -> List[IEndpointConfig]:
        return [IEndpointConfig(route="/dailyreport", class_method=self.enviar_email_kpi_diario, rest_method="POST", tags=["KPI report"])]
    ...
    def enviar_email_kpi_diario(self):
        ...
        for id in ids_loja:  
            ...
            file_name = reportGenerator.generate_report(
                report_type='kpi',
                obj_report_content=formated_content
            )
