...
docker buildx build --platform linux/amd64 --push -t $DOCKER_IMAGE_NAME ./ --build-arg GCP_CREDENTIALS_SECRET_ENCODED=$GCP_CREDENTIALS_SECRET_ENCODED --build-arg EMAIL_SENDER_ACCOUNT=$EMAIL_SENDER_ACCOUNT --build-arg EMAIL_PASSWORDS=$EMAIL_PASSWORDS --build-arg INSTAGRAM_SCRAPPER_API_HOST=$INSTAGRAM_SCRAPPER_API_HOST --build-arg INSTAGRAM_SCRAPPER_API_TOKEN=$INSTAGRAM_SCRAPPER_API_TOKEN
